...
(function( $ ){
    ...
    var methods = {
        ...
        resize : function( event ) {
            ...
            if (leftHeader) {
                $(leftHeader.children().find('tr')).each(function (i) {
                    var tdDivs = $(table.find('tr')[i]).children().children();
                    var maxHeight = 0;
                    var height = 0;
                    var width = 0;
        ...
        scroll : function( event ) {
            ...
            if (topHeader) {
                var tableBottom = tableTop + table.height() - topHeader.height()
                    - table.find('tr')[table.find('tr').length-1].offsetHeight;
            }
            if (leftHeader) {
                var tableRight = tableLeft + table.width() - leftHeader.width()
                    - table.find('tr')[0].cells[table.find('tr')[0].cells.length-1].offsetWidth;
            }
