$(document).ready(function () {
    ...
    function readyView() {
        ...
        pmLine.attr('src', `${supersetUrl}${formatData('pmLine')}${gasLineUrl}`)
         ...
         $('.aqiLineBox').append(aqiLine)*/
           ...
           const div1 = $('<div id="PM25" style="width: 100%; height: 290px; float: left"></div>')
            $('.pmLineBox').append(div1)
           const div2 = $('<div id="AQI" style="width: 100%; height: 290px; float: left"></div>')
            $('.aqiLineBox').append(div2)
    ...
    function setEcharts1(startTime, endTime, cityCode,days) {

        var url = window.url.getForcastAudateCityData + "?startTime=" + startTime + "&endTime=" + endTime + "&cityCode=" + cityCode+ "&day=" + days;
        GetAjax(url, undefined, $(".operate-wrapper")).done(function (res) {
            console.log("遮住---->")
            console.log(res)
            var dataAQI=[];//实测
            ...
            var dataUser=[]//专家名
            var dataTime=[];
            $.each(res.data,function(j,value){
                dataAQI.push(value["AQI_R"])
                datafAQI.push(value["AQI"])
                dataPM25.push(value["PM25_R"])
                datafPM25.push(value["PM25"])
                dataUser.push(value["username"])
                dataTime.push(value["datetime"])
            })
            setOptionEcharts1("AQI",dataAQI,datafAQI,dataUser,dataTime)
            setOptionEcharts1("PM25",dataPM25,datafPM25,dataUser,dataTime)
        });
    }

    function setOptionEcharts1(gas,data,dataf,dataUser,dataTime){
        var options=echarts4.init(document.getElementById(gas));//获取页面折线柱状图存放div
        var dataAll=getDate(data,dataf)
        var option = {
            color: ['#000', '#4A67CF', '#E35263', '#76C76F'],
            barMaxWidth:10,
            tooltip: {
                formatter: function (params) {
                    var res = '';
                    var i=params[0].dataIndex;//获取当前显示的第几个数据
                    res+='主班：'+dataUser[i];
                    res+='<br/>实测：'+data[i];
                    res+='<br/>预测范围：'+dataf[i];
                    return res;
        ...
        options.setOption(option);//添加页面折线柱状图
    }
    //处理数据生成：预测低值--作为辅助悬空数据、预测高值（高于值）、预测高值（低于值）、预测高值（命中值）
    function getDate(data,dataf) {
        var dataAll=[[],[],[],[]];
        for(var i=0;i<data.length;i++){
           var maxF=parseInt(dataf[i].split("~")[1]);//预测最大值
           var smalF=parseInt(dataf[i].split("~")[0]);//预测最小值
            if(maxF<smalF){
                 maxF=dataf[i].split("~")[0];//预测最大值
                 smalF=dataf[i].split("~")[1];//预测最小值
            }
            //辅助值
            dataAll[3][i]=smalF;
            //判断高于值
            if(data[i]<smalF){
                dataAll[0][i]=maxF-smalF;
                dataAll[1][i]='-';
                dataAll[2][i]='-';
            }
            //低值判断
            if(data[i]>maxF){
                dataAll[0][i]='-';
                dataAll[1][i]=maxF-smalF;
                dataAll[2][i]='-';
            }
            //命中判断
            if(data[i]<=maxF && data[i]>=smalF){
                dataAll[0][i]='-';
                dataAll[1][i]='-';
                dataAll[2][i]=maxF-smalF;
            }
    ...
    showData(startTime, endTime, cityCode);
    setEachers(startTime, endTime, cityCode,days);
    setEcharts1(startTime, endTime, cityCode,days);
    // 站点选择
